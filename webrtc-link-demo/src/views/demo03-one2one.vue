...
export default {
  ...
  created() {
	...
	// 流程一：加入房间
	// 1. 携带roomId和useId进入页面后先建立ws连接
	// 2. 服务端发现有新连接进来后先通知房间内的其他所有成员，抛出一个type=join的msg事件
	// 3. 客户端收到jion msg事件后向后端抛出roomUserList事件
	// 4. 服务端收到roomUserList事件后返回房间内所有成员的列表并渲染

	// 流程二：发起一对一通话
	// 1. 发起方A先初始化peerconnection对象
	// 2. 获取本地媒体流添加到pc中，并放入视频组件中
	// 3. 创建pc的监听事件
	// 4. pc创建offer并通过ws的自定义offer事件发送给后端
	// 5. 发送call事件
	// 6. 对应步骤4，后端查询房间内的被呼叫方B的socket，通过该socket（ws生成的）触发被呼叫方的type=offer msg事件
	// 7. 对应步骤5，后端触发被呼叫方的type=call msg事件
	// 8. 对应步骤6，被呼叫方pc创建answer并触发后端answer事件
	// 9. 对应步骤7，被呼叫方
	// 8. 后端触发发起方的type=answer msg事件
  },
  methods: {
          ...
          res.forEach((report) => {
            ...
            if (report.type === "outbound-rtp" && report.kind === "video") {
              ...
              if (lastResultForStats && lastResultForStats.has(report.id)) {
                ...
                let hbf =
                  headerBytes -
                  lastResultForStats.get(report.id).headerBytesSent;
                let pacf =
